...
test("adds a missing key to existing frontmatter", () => {
  const out = applyFrontmatterPatch(baseSpec, [{ key: "verified-by", value: "_none_" }]);
...
test("updates an existing key", () => {
  const out = applyFrontmatterPatch(baseSpec, [{ key: "status", value: "approved" }]);
...
test("is idempotent — applying the same patch twice yields the same result", () => {
  const once = applyFrontmatterPatch(baseSpec, [{ key: "verified-by", value: "_none_" }]);
  const twice = applyFrontmatterPatch(once, [{ key: "verified-by", value: "_none_" }]);
...
test("throws when content has no frontmatter block", () => {
  ...
  expect(() => applyFrontmatterPatch(bad, [{ key: "id", value: "x" }])).toThrow(/frontmatter/i);
});
