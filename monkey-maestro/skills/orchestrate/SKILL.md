...
6. Before creating any workspace, read
   [`references/dispatch.md`](references/dispatch.md). Apply its placement, atomic
   create-or-reuse, worker-prompt, launch, and recovery contract to the selected issues.
   Settle sibling workspace attempts independently and never backfill within this pass.
7. Never poll a worker or refresh Linear for another batch. Report each selected issue from
   the confirmed transport result only.
