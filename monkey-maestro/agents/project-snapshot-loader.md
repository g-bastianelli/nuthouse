...
1. Fetch the exact project and all project comments. Parse versioned graph receipts and
   ...
   are `CONTROL_AMBIGUOUS`. Return every parsed control field in both modes, including
   `decisionBaseline`, `executionIssueIds`, and `exitedExecutionIssueIds`; `full` must
   never project away fields returned by `control-only`. In `control-only`, return after
   this step.
