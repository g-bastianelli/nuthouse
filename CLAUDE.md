...
| `git-gremlin`    | Contextual review, ambient review-comment discipline, and commit + PR drafting with scoped mutation gates. The reviewer loads repository context, then investigates with its own judgment rather than a prescribed pass taxonomy. A reply announcing a fix never precedes the push of that fix, and any review comment an acting agent dismisses receives an explanatory reply before thread resolution. The `commit` and `pr` skills read the staged diff / git log, draft the message, and run `git commit` / `git push` + `gh pr create` themselves after approval — no drafter subagent sits between the gate and the mutation. It owns no workspace orchestration or branch guard. | —                                        | `commit`, `pr`, `review`, `handle-review-comments`                                                                                                      | `reviewer`                                                                                                       | `git-gremlin/persona.md`    |
...
| `moon-moth`      | Moon-aware loop engine for TypeScript monorepos. Scopes work to the `affected` project graph (`moon query`), verifies via affected `:typecheck`/`:lint`/`:test` (evidence over assertion), and wires any moon repo via `init`. Does **not** implement — hands the artifacts to the main implementation turn (`subroutine` discipline skills activate ambiently). SessionStart hook injects affected scope when a `.moon/` workspace is detected.                                                                                                                                                                                                                                        | SessionStart                             | `scope`, `verify`, `init`                                                                                                                               | `affected-scout`, `verify-runner`, `change-auditor`                                                              | `moon-moth/persona.md`      |
| `monkey-maestro` | Linear-first Superset orchestrator. Live Linear status and `blockedBy` relations are the sole scheduling authority; a disposable invocation-local cache enables targeted refreshes. Ready work fans out with scoped runtime/idempotence checks, all-settled failures, resilient locking, and issue-scoped force. Reconciliation is optional runtime-only recovery. No private queue, no hidden daemon, and no hook intercepting your Git commands.                                                                                                                                                                                                                                      | —                                        | `status`, `start`, `orchestrate`, `reconcile`, `spawn`, `stop`                                                                                          | `control-loader`, `project-snapshot-loader`, `runtime-inspector`                                                 | `monkey-maestro/persona.md` |
