...
Personal Claude Code (sometimes Codex) plugin marketplace. Nine plugins right now: `saucy-status`, `react-monkey`, `linear-devotee`, `acid-prophet`, `warden`, `git-gremlin`, `lore-hound`, `stack-golem`, `moon-moth`.
...
| `moon-moth`      | Moon-aware agentic dev loop for TypeScript monorepos. Scopes work to the `affected` project graph (`moon query`), implements TDD-first bounded to the touched packages, verifies via affected `:typecheck`/`:lint`/`:test` (evidence over assertion), and wires any moon repo via `init`. SessionStart hook injects affected scope when a `.moon/` workspace is detected.                                                                                           | SessionStart                   | `scope`, `implement`, `verify`, `init`                                                           | `affected-scout`, `verify-runner`, `change-auditor`                                                     | `moon-moth/persona.md`      |
