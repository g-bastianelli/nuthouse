...
| `monkey-maestro` | Linear-first Superset orchestrator. Live Linear status and `blockedBy` relations are the sole scheduling authority; a disposable invocation-local cache enables targeted refreshes. The execution agent is discovered from the target host (`superset agents list`) instead of assumed, and re-checked before orchestration launches. Ready work fans out with scoped runtime/idempotence checks, all-settled failures, resilient locking, and issue-scoped force. Reconciliation is optional runtime-only recovery. No private queue, no hidden daemon, and no hook intercepting your Git commands. | —                                        | `status`, `start`, `orchestrate`, `reconcile`, `spawn`, `stop`                                                                                          | `control-loader`, `project-snapshot-loader`, `runtime-inspector`                                                 | `monkey-maestro/persona.md` |
