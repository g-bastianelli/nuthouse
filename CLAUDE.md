...
**No backwards compatibility. These are personal plugins and their author is the only
installer.** Nothing here has to keep working for an old install, a retired skill or agent
name, or a past architecture. When a decision turns out wrong, replace it outright; when
something is retired, delete it and every reference to it in the same pass. Never write a
compatibility shim, an alias for an old name, a deprecation path, a migration note in
shipped plugin files, or a gate that defends a past decision — there is nobody on the other
side of it. A removal only has to be **complete**; it never has to be **defended**.
...
- ❌ Gates that assert **inventory** instead of **properties** — a checker requiring a given skill/agent/hook/path to exist, forbidding a retired path, or regex-scanning for retired names → **a gate may only assert what is invisible on read**. Inventory gates freeze one refactor's architecture and make the next one harder: `check-workflow-migration.mjs` literally _required_ the Maestro branch-guard hook that a later refactor removed. `scripts/check-plugin-invariants.mjs` is the surviving shape — manifest version parity and "no durable local state outside the lock", nothing about which files exist.
- ❌ Re-asserting a plugin's own skill/agent contract from a repo-level script → **assert it in that plugin's `tests/`**, where the contract lives. The central copy drifts, doubles the edit on every wording change, and fails far from its owner.
...
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | --------------------------- |
| `subroutine`     | Ambient implementation discipline for TS monorepos. A hook injects the discipline (type-safety, Zod validation, code-organisation, React rules, form rules, testing, state machines, Hono pipeline, Result/unwrap) as context whenever matching files are **edited** — deterministic delivery the **main agent** implements against natively (model-driven skill auto-load was unreliable; the hook fixes that). Code review is left to the runtime's native reviewer. SKILL.md files hold the rules; `check-folder-shape` reviews settled owner folders after structural changes and before verification. No agents; `moon-moth:verify` runs checks in moon repos.      | PostToolUse, SessionStart      | `type-safety`, `validation`, `code-organisation`, `form-rules`, `react-rules`, `testing-discipline`, `state-machine`, `hono-pipeline`, `result-pattern`, `check-folder-shape` | —                                                                                                                | `subroutine/persona.md`     |
| `linear-devotee` | Linear context with decision sources and issue-scoped planning; complete deliverables with exact Acceptance, justified dependencies, and independent behavioral review. Recoverable creation replays one authorized mutation envelope and verifies the actual Linear graph.                                                                                                                                                                                                                                                                                                                                                                                              | SessionStart, UserPromptSubmit | `greet`, `plan`, `create-project`, `create-milestone`, `create-issue`, `next-issue`                                                                                           | `issue-context`, `issue-drafter`, `milestone-drafter`, `plan-auditor`, `project-drafter`, `project-graph-loader` | `linear-devotee/persona.md` |
| `acid-prophet`   | Spec-driven development pipeline. Codebase discovery → EARS spec with stable `AC-###` ids → independent behavioral readiness audit → optional `write-plan` with dependency-ordered AC coverage, contracts, codebase map, and quickstart evidence → handoff to `linear-devotee` or implementation. Project constitution articles become extra gates. PR drift and QA checklists preserve the same AC ids; `[NEEDS CLARIFICATION:...]` markers prevent invention.                                                                                                                                                                                                          | —                              | `write-constitution`, `write-spec`, `audit-spec`, `write-plan`, `write-checklist`, `check-drift`, `reconcile-drift`                                                           | `spec-auditor`                                                                                                   | `acid-prophet/persona.md`   |
| `git-gremlin`    | Ambient review-comment discipline plus commit + PR drafting with scoped mutation gates. Code review is left to the runtime's native reviewer. A reply announcing a fix never precedes the push of that fix, and every review comment an acting agent excludes, judges invalid, rules out of scope, or otherwise refuses to act on receives an explanatory reply before thread resolution. The `commit` skill preserves the existing index or stages dirty changes for a real commit; `commit` and `pr` then draft and run `git commit` / `git push` + `gh pr create` themselves after approval, with no extra gates. It owns no workspace orchestration or branch guard. | —                              | `commit`, `pr`, `handle-review-comments`                                                                                                                                      | —                                                                                                                | `git-gremlin/persona.md`    |
...
| `stack-golem`    | Notom-stack ops & debug toolbox — Scaleway inspection (`scw`) with writes through the owning infrastructure system, platform observability (logs/metrics/health on staging+prod), local dev debugging, Insomnia collection sync. Investigate-first, CLI-driven, never punts to the console.                                                                                                                                                                                                                                                                                                                                                                              | —                              | `debug-local`, `observe-platform`, `drive-scaleway`, `sync-insomnia`                                                                                                          | `platform-scout`                                                                                                 | `stack-golem/persona.md`    |
