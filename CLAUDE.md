...
| `acid-prophet` | Structured spec-writing + audit + drift detection. Q&A → spec → `scryer` audit → optional handoff to `linear-devotee`. PR-time drift check via `check-drift` | — | `write-spec`, `check-drift`, `audit-spec` | `spec-auditor` | `acid-prophet/persona.md` |
