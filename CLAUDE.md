...
| `monkey-maestro` | Durable Linear-backed Superset orchestrator. It hydrates verified graph/runtime state once per coordinator session, launches every ready issue up to bounded concurrency, monitors exact terminals, and advances through targeted Linear reads. Full reconciliation is explicit recovery; manual spawn remains the branch-guard fallback. No private queue or hidden daemon.                                                                                                                                                                                                     | PreToolUse (Bash)                        | `status`, `start`, `orchestrate`, `reconcile`, `spawn`, `stop`                                                                                          | `project-snapshot-loader`, `runtime-inspector`                                                                   | `monkey-maestro/persona.md` |
