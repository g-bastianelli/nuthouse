...
The explicit network policy is limited to `stack-golem:platform-scout`. The original decision
also covered local workspace writes for `moon-moth:verify-runner` and mutation permissions for
`git-gremlin:pr-drafter` and `git-gremlin:commit-drafter`; those agents were later removed when
their work was inlined into skills. Parent runtime permission overrides and approval gates still
apply. Carry Claude tool allowlists, skill preloads, and turn caps into the developer instructions
because Codex custom-agent TOML does not provide one-to-one fields for all of those Claude
settings.
