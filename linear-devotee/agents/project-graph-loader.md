...
already confirmed Linear ids. `PROJECT_ID` is the only project boundary to query.

## Mission

1. Read the chain-state and require `mutation_envelope`, `normalized_graph`,
   `approved_payload_hash`, `graph_hash`, and matching `project.id`. Run
...
2. Fetch the project with milestones and list every issue in that project. Follow
...
3. Correlate entities using confirmed ids first, then the exact hidden marker
...
4. Build normalized entity fields from the fresh Linear response, never from their
...
5. Emit the normalized graph with `dependentRef -> blockerRef` edges. Include every
