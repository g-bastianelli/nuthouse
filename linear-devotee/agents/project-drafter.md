...
LINEAR_CONTEXT: <current raw metadata plus affected issue bodies, decision comments and dependency closure for existing work>
RELEVANT_FILES:
...
5. Apply `shared/coordination-review.md` from the active plugin root to shared contracts and
   coupled modifications, including changes in common files. Return its `COORDINATION_REVIEW`
   with evidence in both modes; review the current state separately from any proposed repair.
   Add dependencies only when the dependent cannot deliver its acceptance without the blocker's
...
**Coordination**
<interaction, resolution, decision source, required output or independence reason, verification owner>
